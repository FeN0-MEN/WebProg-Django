...
from crud.forms import BookForm, UserForm, OrderForm, CartItemForm
from django.http import JsonResponse
from django.core.exceptions import ValidationError
from django.core.validators import validate_email


# Adds user to db from form.
...
# Check if user is authentificated by
# checking his username in request.session dictionary.
def authentificated(request):
    return 'username' in request.session


# Returns role of user form db
...
# Deletes user cookies from current session.
def logout(request):
    try:
...
# Checks is we have user in db.
# Redirectes to index if we have
...
# BookListView returns list of objects and paginates
# to 10 elements by default.
# Returns current role and auth status to index template.
...
# Rewrites record of book in db from form.
...
# Deletes record of book from db.
...
# Returns zipped list of books based on cart table and
# cart items for current user. Also calculates current price for all books.
